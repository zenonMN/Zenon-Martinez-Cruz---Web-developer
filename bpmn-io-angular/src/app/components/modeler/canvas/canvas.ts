...
scroll(delta) {
  ...
  if (delta) {
    console.log("[canvas] - scroll: ", delta, this._viewport, node.getCTM());
    let self = this;
    super._changeViewbox(function() {
      delta = assign({ dx: 0, dy: 0 }, delta || {});
      console.log("[canvas] - translate: ", this._svg.createSVGMatrix().translate(delta.dx, delta.dy));
