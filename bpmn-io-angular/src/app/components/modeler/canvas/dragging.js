...
export default function Dragging(eventBus, canvas, selection, elementRegistry) {
  ...
  function move(event, activate) {
    ...
    console.log("[dragging] -  event: ", event, context, context.payload.x, context.payload.y, context.data.shape, _canvas.viewbox());
    //
    let heightAprox = context.data.shape.height/2 + 10;
    let widthAprox = context.data.shape.width/2 + 10;
    let viewportWidth = _canvas.viewbox().width;
    let viewportHeight = _canvas.viewbox().height;
    
    if(!isNaN(viewportHeight) && !isNaN(heightAprox) && !isNaN(context.payload.y)) {
      console.log("Y: ", context.payload.y, heightAprox);
      console.log("X: ", context.payload.x, widthAprox);
      
      if(context.payload.y <= heightAprox ) {
        end(event);
        return;
      }
      if((context.payload.y + heightAprox) >= viewportHeight || (context.payload.x + widthAprox) >= viewportWidth) {
        end(event);
      return;
      }
    ...
    //   end(event);
    //   console.log("[dragging] -  context: ", context);
    //   let svg = _canvas._svg;
    ...
    //   svgAttr(svg, { width: (width + 100)+'px', height: (100+heigth)+'px'});
    //   return;
    // }
  ...
  function init(event, relativeTo, prefix, options) {
    console.log("[Dragging] - INIT", event, relativeTo, prefix, options);
